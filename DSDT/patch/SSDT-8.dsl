 ...
 *
 * Disassembly of SSDT-8.aml, Mon Sep 18 07:36:32 2017
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002250 (8784)
 *     Revision         0x01
 *     Checksum         0x7E
 *     OEM ID           "COMPAL"
...
DefinitionBlock ("", "SSDT", 1, "COMPAL", "CRV ORB ", 0x00001000)
{
    ...
    {
        OperationRegion (VBOR, SystemMemory, 0x9AFAA018, 0x00007C04)
    ...
    If (\_OSI ("Windows 2013"))
    {
        ...
        PowerResource (AMD3, 0x00, 0x0000)
        {
            ...
            OperationRegion (DGPU, SystemMemory, \_SB.PCI0.RP05.PEGP.EBAS, 0x50)
            ...
            Method (_ON, 0, NotSerialized)  // _ON_: Power On
            {
                ...
                If (LEqual (MSD3, 0x03))
                {
                    ...
                    Store (0x07, PCMR)
                    Store (Zero, PMST)
                    While (LEqual (\_SB.PCI0.RP05.PEGP.DVID, 0xFFFF))
                    {
                        Sleep (One)
                    }

                    If (LAnd (LEqual (GPU3, Zero), LEqual (PID3, One)))
                    {
                        Store (0x063F1028, SSSV)
                    }
                    ElseIf (LAnd (LEqual (GPU3, Zero), LEqual (PID3, Zero)))
                    {
                        Store (0x06401028, SSSV)
                    }
                    Else
                    {
                        Store (0x063E1028, SSSV)
                    }
